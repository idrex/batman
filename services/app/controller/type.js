...
class TypeController extends Controller {
  async index() {
    ...
    const res = await model.Type.findAndCountAll(query);
    res.size = toInt(size);
    res.page = toInt(page);
    success({ ctx, res });
  }
  ...
  //   ctx.body = await ctx.model.Type.findAll(query);
  // }
  ...
  async show() {
    ...
    const res = await model.Type.findByPk(toInt(params.id));
    success({ ctx, res });
  }
  ...
  async create() {
    ...
    const res = await model.Type.create({ name, url, icon, parentId });
    ctx.status = 201;
    success({ ctx, res });
  }
  ...
  async update() {
    ...
    const menu = await model.Type.findByPk(toInt(id));
    if (!menu) {
      ctx.status = 410;
      return;
    }
